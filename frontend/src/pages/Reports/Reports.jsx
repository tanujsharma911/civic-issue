...
function Reports() {
    ...
    useEffect(() => {
        const extractUniqueCity = (data) => {
            ...
            const citySet = new Set(data.map(complaint => complaint.city));

            // Convert the Set back into an array before returning.
            // console.log("in func", Array.from(citySet));
            return Array.from(citySet);
        };

        reportService.getReportsByState(slug).then((res) => {
            if (res.status === 'success') {
                setStateReports(res.data)
                const temp = extractUniqueCity(res.data);
                setAllCity(temp);
            } else {
    ...
    return (
        <div>
            <h1 className="text-4xl mb-4 dark:text-white">All Cities</h1>
            <ul className='flex flex-wrap gap-4 mb-8'>
                {allCity.map((city, index) => (
                    <li key={index} className=''>
                        <Link to={`${city}`} className="flex flex-col p-3 bg-white border border-gray-200  hover:scale-[1.02] overflow-clip transition-all duration-200 hover:border-gray-400 shadow-2xs rounded-xl dark:bg-neutral-900 dark:border-neutral-700 dark:shadow-neutral-700/70">
                            {city}
                        </Link>
                    </li>
                ))}
            ...
            <h1 className="text-4xl mb-4 dark:text-white">All {slug} Reports</h1>
