...
function CityRanking() {
    ...
    useEffect(() => {
        const fetchReports = async () => {
            const res = await reportService.getReportsByState(state);
    ...
    return (
        <div>
            <h1 className="text-4xl dark:text-white">City Ranking of {state}</h1>
            ...
            <div className=" flex flex-col gap-4">
                {rankedCities.map((city) => (
                    <Link to={`${city.name}`} className='' key={city.name}>
